...
public final class Result<T> implements Serializable {
    private static final long serialVersionUID = -2012166212834233795L;
    ...
    public static <T> Result<T> failed(String message) {
        return of(CommonCodeEnum.FAILURE.getCode(), message);
    }


    public static <T> Result<T> failed(T data, String message) {
        return Result.<T>failed(message).with(data);
    }
    ...
    public static <T> Result<T> validateFailed(String message) {
        return of(CommonCodeEnum.PARAM_ERROR.getCode(), message);
    }
